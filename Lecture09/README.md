...
And that's it! You're ready to start with this lecture's exercises! **Happy coding** :star_struck:
...
```
pip install drfp
pip install ipykernel  # to make jupyter notebooks work in VSCode
```
Open the notebook `Part1/drfp_partA.ipynb` and select the kernel for the environment `drfp_pypi`. You will have to install `iPython` again, just as you did in the exercises for Lecture 3. See that [README](../Lecture03/README.md) for help.
...
1. Clone the GitHub repository by running the address you find on
   [https://github.com/reymond-group/drfp/](https://github.com/reymond-group/drfp/)
   under `Code`. One potential folder to place it would be `~/git`, but you can also put
   it into another folder. The folder has to be created before you can navigate there
   with `cd`. It should be outside your exercises folder, and not inside another repo.

![Clone address](../assets/Lecture09/clone_drfp.jpg)
...
Now you can open and execute the code cells in `Part1/drfp_partB.ipynb`. 
...
Next follow the instructions in `Part2/from_script_to_package.ipynb` to learn more about how to build your first package using `pyproject.toml`. Remember that the build tool will only recognize subfolders containing an `__init__.py` file (even if empty) as part of the package. 
